---
title: "midas demo slides"
author: "LCB"
output:
  revealjs::revealjs_presentation:
    theme: night
    highlight: zenburn
    # Nicer, but larger and doesn't render on Drive:
    #
    self_contained: false
    reveal_plugins: ["notes","search","menu"]
    #
    # Self-contained html that hopefully renders on Drive:
    # self_contained: true
---
...
For this demo (scroll for full attribution, license):
...
<!-- FIXME discuss completion approaches in epidatr material; grouped complete & joint complete -->
...
jhu_csse_daily_subset %>%
  ...
  mutate(facet = dplyr::recode_factor(signal,
                                      cases = "Cases",
                                      cases_gr1 = "Case Growth Rate",
                                      cases_gr2 = "Case Growth Rate")) %>%
...
cors =
  jhu_csse_daily_subset %>%
  epi_cor(death_rate_7d_av, case_rate_7d_av, dt2 = -21L, cor_by = "time_value", method="kendall")
cor_facets =
...
## TODO Dan archive slides

## `epiprocess::epix_slide`

Version-aware counterpart to `epi_slide`.  Performs a time-windowed calculation using snapshot as of each of the requested `ref_time_values`.
```{r}
slide_result =
  archive_cases_dv_subset %>%
  epix_slide(
    function(x, grp) {
      tibble(
        ...
        coefs = x %>%
          ...
          lm(formula = case_rate_7d_av ~ lag(percent_cli, 7L) + lag(percent_cli, 14L)) %>%
          coef() %>%
          list()
      )
...
slide_result_facets = c(
  ...
  "Target data by time_value",
  "Predictor data by time_value"
) %>%
  {factor(., levels=.)}
slide_result %>%
  ...
  geom_line(
    aes(time_value, percent_cli, colour=as_of, group=as_of),
    slide_result %>%
      # transmute(as_of = ordered(as_of), slide_value_dat) %>%
      transmute(as_of = as_of, slide_value_dat) %>%
      unnest(slide_value_dat) %>%
      filter(geo_value == "ny") %>%
      mutate(facet = slide_result_facets[[3L]])
  ) +
...
<!-- FIXME coefficients of intercept and covariate not comparable -->
